...
if not st.session_state["logged_in"]:
    ...
    # 自分宛ての未完了タスクを数える（通知バッジ用）
    my_active_tasks = [t for t in all_tasks if t['to_user'] == current_user and t['status'] != '完了']
    alert_msg = ""
        ...
        for task in my_tasks:
            ...
            with cols[status]:
                with st.container(border=True):
                    ...
                    # 操作ボタン
                    if status == "未着手":
                        if st.button("着手 ➡", key=f"s_{task['id']}"):
                        ...
                        if st.button("完了 ✅", key=f"d_{task['id']}"):
                         ...
                         if st.button("完了 ✅", key=f"rd_{task['id']}"):
                            update_status(task['id'], "完了")
                            st.rerun()
    ...
    # 2. 通知センター（新機能）
        ...
        st.subheader("🔔 通知センター")
        st.write("あなたに関連する最新の動きです")
        if st.button("最新情報を取得"): st.rerun()
        
        # 自分宛てのタスクを日付順に表示
        my_related = [t for t in all_tasks if t['to_user'] == current_user]
        
        if my_related:
            for task in reversed(my_related): # 新しい順
                with st.container(border=True):
                    col_icon, col_text = st.columns([1, 8])
                    with col_icon:
                        if task['status'] == '完了':
                            st.markdown("✅")
                        elif task['priority'] == '🔥 至急':
                            st.markdown("🔥")
                        else:
                            st.markdown("📩")
                    
                    with col_text:
                        st.markdown(f"**{task['from_user']}** からの依頼: 「{task['content']}」")
                        st.caption(f"状態: {task['status']} | 追加日: {task.get('date','--')} | 完了日: {task.get('completed_at','--')}")
        else:
            st.info("通知はありません")
